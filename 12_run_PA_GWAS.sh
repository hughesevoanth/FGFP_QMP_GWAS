...
## There are 3520 jobs (2772 in the Apr 2021 data set)
...
CHR=$(sed "${i}q;d" ${PA_JOBS} | awk -F " " '{ print $1 }')

## extract the trait to be run on this job
TRAIT=$(sed "${i}q;d" ${PA_JOBS} | awk -F " " '{ print $2 }') 
...
time snptest_v2.5.4-beta3 \
	...
	-frequentist 1 \
	-method expected \
	-cov_names age sex PC1 PC2 PC3 PC4 PC5 PC6 PC7 PC8 PC9 PC10 \
	-hwe \
